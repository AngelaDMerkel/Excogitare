...
| [Map Type narrative identities](features/map-type-narrative-identities.md) | Specified | Treat all thirty current Map Types and the approved Three Realms, Thalassic League and Unequal Realms Polis additions as recognizable narrative premises expressed through their owning engine, reinterpreted by World Character, complicated by World Modifier, and constrained by explicit controls, victory geography and Civ V legality. Every type requires a unique narrative verb, mandatory relationships, nearest-confusion boundaries, anti-motifs, retained diagnostics and a blind-recognition statement. Twenty approved concrete display names replace nebulous labels while stable machine IDs and the names Lonely Oceans, Dynamic Earth, Imperial Ring, Three Realms, Thalassic League and Unequal Realms remain unchanged. | The naming migration is verified across runtime labels and descriptions, generated default names, narrative headings and cross-references, Identity Lab, README/gallery assets, tests, Pages and Alpine runtime. The narrative generator identities and three additional Polis types remain specification only and must not be claimed as implemented. |
| [Identity Lab](features/identity-lab.md) | Verified | A blue Development-labelled Lab workspace runs deterministic blind Map Type recognition sessions beginning with Lonely Oceans, Broken Island Chains, Great Watersheds and Glacial World; it retains exact recipes, human review and structural diagnostics as versioned JSON tied to the narrative guide. | Side-effect-free model, blind deck, reveal and navigation, local persistence, rejecting JSON import/export, summaries, in-interface guide, README, tests, Pages build and Alpine runtime were verified. This does not claim that the four narrative identities themselves are implemented. |
| [Narrative generation rewrite](features/narrative-generation-rewrite.md) | Groundwork | Plan a coordinated rewrite covering Match Intent-aware Polis geography, all thirty-three accepted Narrative Map Types, the five-stage Design → Refine → Iterate → Edit → Review workflow, independent Scale and Archetype systems, continuous four-choice Identity Lab sessions, non-navigating history review, preservation-aware selective regeneration, computation controls and durable authoring provenance. | The architecture and implementation sequence are being specified. No runtime behavior should be claimed until the plan is approved and its individual feature gates are implemented and verified. |
