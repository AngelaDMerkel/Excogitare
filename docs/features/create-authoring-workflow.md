...
- **Status:** Partial. The five-stage navigation and approved control split are live; component extraction and several persistence/accessibility gates remain open.
...
The current monolithic Create UI is split into focused stage components backed by one normalized project/store boundary. Each stage has one clear primary action, shallow progressive disclosure and compact summaries. Tooltips may add detail but never contain the only warning or blocked consequence.
...
- [ ] Create is decomposed into focused components with one normalized state boundary.
...
- [ ] Refine, Edit and Review retain their local state across workspace switching.
- [ ] Progress, cancellation, errors and export confirmation remain clear and atomic.
- [ ] Keyboard, focus, hover-help and responsive behavior are verified.
