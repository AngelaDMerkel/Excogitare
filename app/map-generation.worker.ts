  ...
  try {
    if (event.data.kind === "REGENERATE") {
      ...
      const map = event.data.stage === "CLIMATE" && event.data.recipe
        ? buildArchetypeRefinementCandidate(event.data.map, options, event.data.recipe, event.data.variation)
        : regenerateMapStage(event.data.map, options, event.data.stage, event.data.variation, event.data.recipe);
