...
The top bar is organized around six workspaces rather than six unrelated menus: **Explore**, **Create**, **Scenario**, **Repair**, **Lab** and **Lua**. Selecting a workspace changes the job the sidebar is doing without replacing the current map or disturbing its zoom, pan, layers, history or selections. The switcher gives each workspace a restrained identity—teal Explore, gold Create, violet Scenario, copper Repair, blue Lab and red Lua—while retaining written labels so colour is never the only explanation. Lab carries a blue **Development** label; Lua carries a red **Experimental** label. Neither is a claim of maturity.

Create, Scenario, Repair, Lab and Lua open a dedicated contextual strip beneath the primary header. That strip names the active workspace and stage, exposes only that workspace's stages, and reports useful state such as the generation seed, edit selection, scenario blocker count, repair count, blind-review progress or Lua runtime status. It is deliberately separate from the top-level switcher: “where am I?” and “what am I doing here?” are related questions, not the same control. Returning to a workspace restores the stage last used there.

These stages are navigation, not a compulsory wizard. Create may move freely between **Design**, **Refine**, **Iterate**, **Edit** and **Review**. Scenario uses **Setup**, **Factions**, **World**, **Objectives** and **Validate**. Repair separates **Inspect**, **Correct** and **Validate**. Development Lab separates **Review**, **Results** and **Guide**. Experimental Lua separates **Script**, **Generate** and **Diagnostics**. Explore needs no subordinate stage because inspection is already its entire purpose. Create begins directly with **Randomise** and its working controls rather than repeating the workspace and stage already named above it. Other workspaces retain task mastheads where those add context. Explore keeps the complete map name and description; ordinary working spaces reduce them to a compact **Current map** disclosure. Lab hides the candidate name, description and preset until the reviewer has committed a guess.
...
- **Export Civ5Map** writes the current geography using its edited name as the filename. The map name and description remain separately terminated metadata fields, so Civ V's map list displays the name rather than running directly into the description. Create exports are ordinary map files: they contain terrain, elevation, features, rivers, resources and wonders, but do not fabricate Scenario player records. Civ V assigns civilizations and starting plots when the game is created. Generated start plans, ownership, ruins, barbarian camps, goody huts and roads remain in the downloaded `.excogitare` project rather than being smuggled into a synthetic Scenario block. Existing imported Scenario records are preserved and may be edited only within their fixed binary capacity. A confirmation modal appears if validation finds material problems.
- **Open map** loads another `.Civ5Map`. Unsaved in-memory history is not a substitute for keeping the original file.
- **New project** begins a distinct authoring document from the current generated or imported map. **Save project** chooses a project name and history policy, then downloads a portable `.excogitare` bundle. **Open project** validates a current bundle or migrates a legacy v1 project before replacing the current session. The project status identifies the project separately from the map and marks changes as **Unsaved** until another download succeeds.
...
Desktop controls provide contextual help on hover and keyboard focus. These tooltips explain consequences and scope rather than merely repeating a button label: Create keeps persistent copy to labels, compact state and warnings while moving secondary recipe explanations into this help layer; editor tools state what they change, Repair profiles state how aggressive they are, and export or canvas controls explain their output. The help surface is fixed above the application so sidebar scrolling and nested panels cannot clip it. Touch devices omit the tooltip layer.

### Excogitare project files

A `.Civ5Map` is a clean game file; it cannot retain Excogitare's recipe, Scale, Archetype, Match Intent, narrative evidence, semantic protection, Scenario draft, named checkpoints or authoring history. A `.excogitare` file is therefore the authoritative authoring handoff. Opening a Civ5Map automatically promotes the imported geography into a new unsaved project without modifying the original file. Project name and map name begin related, but remain separate: renaming the authoring project does not silently rename the in-game map.

The current format is a DEFLATE-compressed ZIP bundle containing a readable manifest, clean `map.civ5map`, authoritative map and recipe records, protection, Scenario draft, derived evidence, editor state, history index and separately replaceable snapshots. The manifest records each payload's expanded size and SHA-256 hash. Import rejects missing or duplicate entries, path traversal, executable content, unsupported compression or required capabilities, unsafe entry counts and sizes, checksum failures and future bundle contracts before replacing the active project. Safe optional data under `extensions/` and unknown optional JSON fields survive load and resave. Legacy monolithic schema-v1 JSON imports through a pure migration and is written back only in the current format.

**Full history** includes the current map, up to thirty retained generations and every named checkpoint. **Current + checkpoints** omits ordinary generation history for a materially smaller bundle. Both policies preserve the active recipe, protection, Scenario draft and editor continuity. A 64 MB limit applies to both compressed input and expanded payloads. If full history exceeds it, save the current map and named checkpoints instead.

Downloaded files are the only durable project contract. Excogitare has no account, cloud save or server project store, and browser state is not described as saved work. Closing or reloading with unsaved project changes produces a warning; the remedy is to download the project and reimport that file during a later clean session.
...
**Scale** declares how much of an imagined world the tile budget depicts. It is independent of **Map size**: Scale changes the category and frequency of geographic systems, while Map size changes how many tiles render the selected category. A Standard Local map is therefore not a shrunken planet; it is a detailed fragment of one larger system. Conversely, a Duel Global map is a coarse rendering of several systems rather than a close view with fewer tiles.

| Scale | Intended subject | Generative consequence |
|---|---|---|
| **Global** | Most of a planet | More independent land, plate and basin systems; broad climate span; long hierarchical drainage. |
| **Continental** | A continent or connected continental system | Fewer independent systems with greater internal continuity and a still-substantial climate range. |
| **Regional** | Connected countries, basins or seas | One related geographic system with balanced major structure and local detail. |
| **Provincial** | One subcontinental theatre | Large partial ranges and basins, narrower climate window, stronger corridor-scale detail. |
| **Local** | A valley, island group or scenario region | A close section of a larger system: few major causes, dense local relief and short drainage hierarchy. |

The interpretation is engine-specific rather than a cosmetic multiplier. Excogitare changes field span and the number of independent warped centres. Eccentric changes continent/basin grammar while retaining progressively denser polygon and subregion detail. Physical changes the number and sampled extent of tectonic systems, relief wavelength, erosion detail and climate window. Polis changes protected territorial scale, route wandering and chokepoint pressure. Each result records Scale diagnostics, and deterministic tests cover every engine at every Scale. Some Narrative Map Types will receive still more specialized Scale reinterpretations in the later narrative-compiler phases; Phase 3 does not falsely claim those unwritten identities.

**Generation effort** selects a deterministic candidate budget: Standard evaluates one candidate, Thorough evaluates four and Exhaustive evaluates twelve before retaining the best legal result. Mobile generation is deliberately restricted to Standard effort and safe map budgets.
...
The complete [Map Type narrative reference](docs/features/map-type-narrative-identities.md) defines the premise, unique verb, mandatory relationships, anti-motifs, nearest confusions and blind-recognition statement for all thirty-three selectable types. All twenty-six non-Polis types compile retained geographic relationships and final-map assessment before starts and content. The seven Polis types compile separate strategic programs against Match Intent rather than sharing one renamed conflict pattern.

The compiler does not reduce these identities to water and mountain presets. Lake Kingdoms hardens the enclosing land boundary around internal waters; Land and Sea Maze builds tortuous chambers and corridors; Ecological Transect composes coast, marsh, plain, mountain wall and rain shadow as a causal sequence; Wonder Heartlands moves compatible value into exceptional inner regions; Volcanic Island Arcs couples shelves, volcanic anchors and sheltered seas; Inland Supercontinent is oceanless and drains inward. Review scores the final tiles—not the existence of metadata—and discloses weak motifs, forbidden similarities and explicit settings that pulled the result away from its premise.

Map Type controls now begin at each identity's approved water and mountain defaults. Randomise samples that identity's ordinary envelope instead of throwing an arbitrary global water percentage at it; a World Modifier or explicit slider may still create a deliberate contradiction. Excogitare honors the explicit control, records the conflict and lowers the recognition assessment rather than secretly undoing the user's choice.
...
| Eccentric | Lonely Oceans | Vast empty oceans confine each major civilization to a distant viable island realm. |
...
| Physical | Inland Supercontinent | An oceanless continental world drains inward from peripheral highlands toward a remote, occasionally lacustrine heart. |
...
| Polis | Three Realms | Three equal strategic realms each border both rivals through shared theatres. |
| Polis | Thalassic League | Coastal powers compete through redundant sea lanes, ports and contestable city-state diplomacy. |
| Polis | Unequal Realms | Disclosed Tall, Wide, War and Turtle territories impose deliberately different strategic obligations. |
...
*The original four Polis samples. The runtime catalogue now also includes Three Realms, Thalassic League and Unequal Realms; starts, roles, objectives and protected routes are compiled before terrain.*
...
The **Strategy graph** layer renders that retained model over the map: teal safe territories, gold contested or objective regions, and distinct land, pass and naval front lines. **Review** reports front counts, route width and redundancy, city-state contestability, realm roles, actual start distance, five separate victory-feasibility findings and every relaxed constraint. Generation history and `.excogitare` projects preserve the graph; exported `.Civ5Map` files do not, because Civ V's format has no ordinary section for Excogitare's private planning model.
...
- **Polis controls** tune symmetry, expansion pressure, naval importance, chokepoint density and safe-territory radius beneath the selected strategic Map Type. Imperial Ring preserves lateral travel around a shared centre; Opposing Fronts builds two sides with plural breaches; Contested Heartland creates many-to-many approaches; Rival Continents provides plural expensive hinges; Three Realms enforces three mutually contacting realms; Thalassic League builds redundant naval and port networks; Unequal Realms assigns disclosed Tall, Wide, War and Turtle roles. Strong AI accommodation widens corridors and adds route redundancy without granting hidden yields. Incompatible Three Realms totals normalize downward with a visible relaxation; Unequal Realms is selectable directly but never appears in ordinary Randomise.
...
Camps, ruins, ruined cities, roads, and generated start positions are scenario content. Excogitare previews and validates them during design and retains them in `.excogitare` projects, but ordinary Create map exports omit them. Civ V assigns starts and its normal setup content when the map is selected. Imported scenarios fare better because supported records can be amended within their existing binary structure rather than invented.
...
Refine contains the controls whose job is to give an established world its inhabitable character. **World Character** still influences the engine's climate and terrain interpretation. **Archetype** is a deterministic, regionally coherent environmental coat that can be changed without redrawing land, water, relief, rivers, starts, cities or ownership. Narrative Default delegates the surface to Map Type and World Character; Existing is the honest default for imported maps. Temperate, Jungle, Sunscourged, Worldfrost, Monsoon, Mediterranean, Steppe, Savanna, Marshland, Volcanic, Jurassic, Post-Collapse and Fallout Wastes each define a climate envelope, weighted terrain and feature palette, compatible World Characters, resource ecology and natural-wonder tendencies. An explicit contradictory combination remains selectable, but the interface discloses the weakened identity instead of silently substituting another choice. An Archetype is not a new continent generator and does not pretend to be one.

**Archetype intensity** controls the intervention. **Hint** repaints a restrained collection of coherent regions. **Strong** makes the coat dominant while leaving portions of the underlying surface legible. **Transformative** repaints the complete eligible surface and regenerates resources and wonders against the selected ecology. Transformative is never chosen by Randomise. Applying a surface pass opens a modal with **Original**, **Preview** and **Difference** views, changed-surface/content counts and an explicit Transformative warning. The candidate is installed only after confirmation; discarding it or encountering an error leaves the current map untouched. Imported improvements, routes and ownership survive the candidate pass, and protected tiles are merged back before acceptance.

Refine separates **Apply surface & climate**, **Apply content** and **Apply players & starts** so a useful topography need not be thrown away to revise one layer. The resulting Scale, Archetype and intensity remain attached to the map and are restored with generation history or a downloaded Excogitare project; older project recipes without intensity normalize to Strong. Randomise chooses only World Character-compatible Archetypes, safe dimensions and Hint or Strong intensity unless the separate game-breaking permission explicitly broadens dimensions. Any selective pass invalidates structural evidence derived from an earlier complete generation until a later full generation rebuilds it.

**Match Intent** records Human, AI and Flexible composition, team intent, competitive strictness, AI accommodation and whether each victory is Disabled, Enabled or Emphasized. It survives history and downloaded project export. Polis consumes the intent while choosing teams, safe-region capacity, objectives, route kinds, route width and redundancy. Other engines use emphasized victories conservatively for start/content rules and report final-map opportunity without pretending to own a strategic graph. Specific civilization seats remain a Scenario concern because an ordinary Civ V lobby may reassign them.

### Iterate: regeneration, candidates and history

- **Generate map** runs all passes in an isolated worker. A shared status panel reports the active pass, keeps cancellation available, and leaves the installed map untouched until a complete result succeeds. Failures remain visible without changing the current Create stage.
- **Generation history** keeps the latest 30 generated maps, including their exact recipe, seed, parent generation and operation, for the current browser session. Opening entries never leaves Iterate, so the list may be browsed continuously. **Use as Design recipe** is the explicit way to copy an older recipe into Design without replacing the current map. Reloading the page clears in-memory history unless it has been downloaded in an Excogitare project.
...
The Create stage tabs follow the standard keyboard pattern: arrow keys move between stages, while Home and End jump to the first and last stage. Each stage retains its own scroll position and expanded disclosures while the user moves through Create; downloaded Excogitare projects preserve those positions, the active stage, canvas view and history branches for later reimport.
...
- **Drag to Preserve** records named protected regions across topology, elevation, climate, features, hydrology, content, starts and scenario channels. The persistent violet overlay makes the retained area visible while authoring and can be hidden independently. Generated geographic systems retain stable semantic lineage; imported maps infer complete river-edge networks, mountain ranges and connected land regions with an explicit confidence and explanation rather than claiming certainty.
- **Semantic protection** offers four materially different promises. **Exact** retains all selected values; **Shape** retains the footprint and relief; **Function** retains the minimum geography needed for the system to work; and **Relationship** retains its anchors and connections to neighbouring objects. **Preserve this Watershed** reconstructs Civ V's directed river-edge graph and protects a complete mountain-to-water drainage function rather than a merely adjacent cluster of river tiles. Hard failures leave the current map untouched; soft degradation is disclosed in the fidelity report.
- Selective regeneration compiles one neutral protection payload into each engine before candidate completion: Excogitare conditions its fields, Eccentric its polygon allocation and graph paths, Physical its sea-level/relief/drainage boundaries, and Polis its strategic anchors, edges and territories. Deterministic candidate selection is followed by an exact Civ V merge and seam reconciliation. The report records the adapter, candidate count, retained invariants, lineage confidence and seam work. Protection changes have their own Undo and Redo history and are stored in downloaded `.excogitare` projects; ordinary `.Civ5Map` files contain only the resulting legal map data.
...
**Narrative recognition** precedes multiplayer balance. Every selectable Map Type grades its required relationships, explains component evidence, discloses controls that weakened the premise and names its nearest confusions. **Match Intent feasibility** then evaluates Domination, Science, Culture, Diplomacy and Time separately. Polis uses retained routes, roles, capacity and city-state contestability; other engines use final starts, resources, production and territorial capacity. These are explanatory heuristics, not changes to Civ V rules or promises about AI behavior. **Multiplayer balance** grades the map and each major start, while **Civ5 validation** lists errors, warnings and informational findings involving dimensions, tile legality, rivers, starts, resources and scenario data. Review reports problems but does not alter them; use Edit or Repair for that.

Generated structural evidence is dependency-aware. Editing a river invalidates hydrology and the reports derived from it without falsely invalidating the retained topology; changing land and water invalidates the whole downstream chain. Review displays a conspicuous stale-evidence notice, its cause and the affected passes. Regenerating the map is explicit because rebuilding evidence from a recipe may also rebuild authored geography.
...
Scenario is where finished geography becomes an authored game setup. It does not pretend that every useful design intention belongs in a `.Civ5Map`: the workspace labels each record family independently as **Read**, **Edit**, **Write** or **Game verified**, preserves unsupported intentions in the downloaded `.excogitare` project, and discloses those omissions again before game-file export. Entering Scenario creates a draft from the current map without mutating the imported source bytes. Its preview supplies the Political, Start locations and City states layers while ordinary pan, zoom and display state remain intact. New Scenario drafts are project-only at the game-file boundary; Excogitare no longer constructs compact Scenario sections after representative exports were rejected by Civ V.

### Scenario workspace stages

- **Setup** separates scenario name and description from the project identity, distinguishes a fixed scenario from a flexible lobby, records the ruleset or mod profile, and declares major- and city-state slot capacity. Intended era, speed, starting turn and calendar are explicitly **Project only**; they guide future mod or Lua compilation but are not smuggled into Civ5Map metadata.
- **Factions** authors stable faction IDs, slot order, civilization and leader identifiers, major or city-state kind, team and team colour, playable state, and Human/AI/Flexible intent. A start can be entered numerically or placed by arming **Place start** and clicking the map. Validation requires every enabled faction to have a legal land start and keeps every major and city-state start at least five hexes from every other one. Imported scenario player blocks have a fixed record count, so their existing slots may be reassigned or reordered but not added, deleted or disabled.
- **World** edits existing city name, owner, population and tile link; political ownership; improvements already declared by an imported file; and roads or railroads. Individual records can be edited directly, while a map-linked hex-radius brush applies or clears ownership, improvement and route layers independently around the hovered tile. Generated maps can declare the supported improvement types they use. New city-record allocation, arbitrary type-table growth, units and unknown mod records remain outside the safe Civ5Map writer because changing their variable-sized blocks without a complete specification would be an excellent way to produce a file Civ V refuses to load.
- **Objectives** associates faction, team or semantic-geography goals with a project. A goal may refer to a retained watershed, strait, range or other generated object rather than a brittle rectangle of tile coordinates. Objectives, victory-rule intent, briefings and faction notes remain **Project only** until a later Lua or mod compiler can represent them honestly.
- **Validate** checks scenario identity, capacities, unique slots and IDs, teams, start bounds and terrain, five-hex separation, city records and links, ownership references, improvement tables, routes, semantic objectives, shared terrain/resource/river legality and the encoded Civ5Map structure. Its compatibility ledger explains the evidence behind every record family. **Send fixable findings to Repair** is an explicit handoff; Validate never mutates the scenario merely because a problem was detected.

**Apply Scenario preview** commits the map-facing preview to ordinary edit history and project state. **Review Civ5Map export** always opens a modal. A newly authored Scenario is blocked from Civ5Map export and must be retained in the downloaded `.excogitare` project; the same geography can still be exported as an ordinary map from Create or Explore. For a Scenario imported from a real Civ5Map, supported fixed records may be encoded against the original binary, structurally inspected and reparsed before download. A successful binary round trip is still called **Write**, not **Game verified**: only representative loading in Civilization V can justify the latter claim.

The Scenario draft travels with the downloaded `.excogitare` project and restores its active Scenario stage. A bare `.Civ5Map` carries only the compatible game-facing subset. That distinction is not especially glamorous, but it prevents an attractive interface from making false promises about a poorly documented binary format.
...
Repair checks dimensions, tile counts and malformed terrain values; illegal terrain, elevation, feature, resource and wonder combinations; fish on land and land resources in impossible terrain; rivers drawn in water, disconnected edge fragments, inland dead ends and drainage that fails to reach an ocean or lake; scenario cities with broken tile links, duplicate identifiers, missing records or impossible placement; and existing Scenario starts outside the map, on blocked terrain, duplicated, fewer than five hexes apart, misflagged as major or city-state starts, isolated by mountains, absent entirely, or inconsistent with the stored player count. An ordinary geography map without embedded starts is not broken: Civ V assigns them when the game is created, and Repair will not convert the file into a Scenario merely to add preview coordinates. A separate binary inspector walks the metadata sections, world-size field, exact tile grid, scenario marker, slot counts, record offsets and encoded coordinates before download; a structurally invalid `.Civ5Map` cannot be bypassed through the ordinary warning confirmation.

Illegal resources are relocated to a compatible nearby tile when a defensible destination exists and deleted when it does not. Illegal features and wonders are removed or corrected according to their rule. River repair may discard broken fragments and rebuild a continuous mountain-to-water network because preserving a nonsensical river more faithfully would still leave it nonsensical. Start repair may replace a complete overcrowded layout, correct city-state flags and synchronize the logical player count when writable player records already exist. Complete-layout repairs are transactional: if another selected correction changes terrain beneath a proposed start or city, Repair replans against the final corrected tiles and leaves the original map untouched when no complete legal layout exists. A zero-start geography-only map is an ordinary map rather than a broken Scenario; Civ V assigns its starts at game creation. A file that already contains Scenario data but exposes no writable player slots remains blocked, because replacing that Scenario wholesale would be a more destructive intervention than Repair pretends to be.
...
The current Lab covers all thirty-three Narrative Map Types. A deterministic scheduler balances targets in shuffled catalogue-length batches. Each trial presents an unlabeled generated map and exactly four unique answers: the true identity plus three plausible confusions drawn first from the narrative guide, then from reciprocal confusions and the same engine. Choice order is independently shuffled. This is stricter than asking the reviewer to browse an enormous dropdown, but more honest than padding the answers with three absurd alternatives.

### Lab workspace stages

- **Review** starts or resumes a continuous session. Choose a World Character, map size and stable session seed. The candidate name, description, seed, target and engine remain hidden. Selecting an answer and choosing **Submit and continue** moves directly to the already prepared next world; there is no correctness feedback between trials. The session continues until the reviewer deliberately chooses **End and export**.
- **Results** is locked while the session is active. Once ended, it reports total answers, accuracy, mean response time, results by identity and the most frequent target-to-answer confusion pairs. A high score is evidence that the narrative is legible; it is not proof of Civ V legality or multiplayer quality.
...
Generation uses bounded memory: the Lab retains only the current rendered map, one prefetched rendered map and compact records for earlier trials. It does not accumulate hidden tile grids in the session JSON. A generation error remains visible and retryable without losing the current answer. Ending a session both freezes the evidence and downloads it before revealing aggregate results.

The active schema-v2 session is retained in browser storage on this device as a convenience. **Download JSON** is the durable development handoff; **Import JSON** restores a compatible session without replacing it when validation fails. A schema-v1 finite-deck export remains importable as a read-only archive. Excogitare does not invent response times, four-choice positions or continuous-session meaning for that older evidence. Future and malformed schemas are rejected.
...
- `configuration` records the stable session seed, World Character, size, modifier and complete target catalogue. It defines the experiment rather than any one map.
- `trials[].targetPreset`, `choices` and `correctPosition` define the blind question. The normal interface conceals the target and score until the session ends; downloaded JSON is a development artifact and does not pretend to remain blind once opened.
- `trials[].options` and `trials[].recipe` retain the exact deterministic input needed to reproduce that world after a generator change. Prior map tiles are deliberately absent.
- `selectedPreset`, `selectedPosition`, `presentedAt`, `answeredAt` and `responseTimeMs` retain the human answer and timing without an intermediate reveal.
- `diagnostics` and `narrativeEvidence` retain structural measurements and the generator's scored motif assessment. They help explain a judgment but cannot replace it: a quota can be technically correct while the map still looks generic.
- `summary.byIdentity` aggregates recognition per narrative. `summary.confusions` lists target and selected pairs, making systematic identity collisions visible; `summary.choicePositions` helps identify answer-position bias.

The intended development loop is prosaic on purpose: end and export a session, attach the JSON or provide its local path in a later task, compare its confusion pairs and retained structural evidence with the narrative guide, then change engine rules rather than individual favorable seeds. Regenerate the exact retained recipes for comparison. A later code change still has to pass legality, accessibility, determinism and World Character tests. The Lab never converts opinion into an automatic generator patch, and the existence of the tool is not itself human recognition evidence.
