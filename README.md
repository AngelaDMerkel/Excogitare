...
The complete [Map Type narrative reference](docs/features/map-type-narrative-identities.md) defines the premise, unique verb, mandatory relationships, anti-motifs, nearest confusions and blind-recognition statement for the thirty current types and three approved Polis additions. Excogitare retains that authoritative profile for every generated map. **Lonely Oceans**, **Broken Island Chains**, **Great Watersheds** and **Glacial World** additionally have dedicated benchmark compilers and component assessments; the other types are plainly marked Profile-only until their specialized engine passes are written. Three Realms, Thalassic League and Unequal Realms remain approved future Polis types rather than selectable generator options.

Those four compilers solve different problems before starts or content are placed. Lonely Oceans allocates one viable island realm per major civilization, spends most of the map on intimidating negative space and removes city states when solitude cannot coexist with them. Broken Island Chains draws several parent arcs, then distributes volcanic anchors, satellites and real gaps along each arc. Great Watersheds retains headwaters, trunk routes, tributary joins, wet lowlands and outlets, and defaults to dense rivers. Glacial World builds irregular ice sheets and temperate refuges, then puts enough strategic and luxury value in the cold frontier that expansion into hostile land is necessary rather than merely foolish.
