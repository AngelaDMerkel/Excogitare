...
The complete [Map Type narrative reference](docs/features/map-type-narrative-identities.md) defines the premise, unique verb, mandatory relationships, anti-motifs, nearest confusions and blind-recognition statement for the thirty current types and three approved Polis additions. All twenty-six non-Polis types now compile their own retained regions, paths, causal effects and final-map assessment before starts and content are placed. The four current Polis types remain plainly marked Profile-only until Match Intent is integrated; Three Realms, Thalassic League and Unequal Realms remain approved future Polis types rather than selectable generator options.
...
**Narrative recognition** precedes multiplayer balance. For a benchmark Map Type it grades the required structural relationships, explains the evidence behind each component, discloses explicit controls that weakened the premise and names the closest identities with which the result could be confused. Profile-only types show an honest unavailable state rather than receiving a decorative score. **Multiplayer balance** then grades the map and each major start. Player cards report a heuristic score, workable nearby land, strategic resources, luxuries and nearest-opponent distance; selecting a player focuses that start on the map. **Civ5 validation** lists errors, warnings and informational findings involving dimensions, tile legality, rivers, starts, resources and scenario data. Review reports problems but does not alter them; use Edit or Repair for that.
